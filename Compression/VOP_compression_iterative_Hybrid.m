function VOP_compression_iterative_Hybrid(filename_in,eps_G,R,Sglobal,name_VOP,use_Sglobal,max_num_VOPs,max_iter)
...
numMat4Pageeig=1000000; %number of matrices used in the pageeig algorithm. Lower number reduces memory requirements.
...
overestimation_factor=max(real(eig(reformat_tri(Sglobal,y))))/real(B(1))*100; %Calculate the maximum overestimation for information purposes.
...
iteration_step=1;
% if use_GPU==true
%     gMatrices=gpuArray(matrices); %Send matrices to GPU. They will be kept there to reduce overhead. I recommend using single precision matrices.
% end
...
while (num_vops<max_num_VOPs) && (iteration_step<=max_iter)
    ...
    while a<N %Evaluate all matrices (voxels). 1st has already been put in.
        ...
        if num_vops<=N_vop_switch || inner_runs==1
            ...
            V_sub_double=double(V_sub); %Here we use the sub volume WITHOUT overestimation, so that we get a result with minimum total overstimation
            ...
            if feval>0 %If not upperbounded, add it to subset.
                ...
                P_pre2=sum(V_sub.*(c_wv/sum(c_wv)),2); %Calculate the weighted sum of the VOPs here
                if num_vops<=N_vop_switch
                    I_eigen(1)=[];
                end
                ...
                if use_GPU==true %GPU enabled version
                    ...
                    disp(['Dropped ' num2str(dropped_mats) ' of ' num2str(N_save) ' matrices in first round (Cholesky).'])
                ...
                elseif use_chol %Version for CPU using Cholesky Factorization
                    ...
                    while kappa<N %This is not a parfor loop since "chol" doesn't work well in a parallel loop.
                        ...
                        if isPSD(kappa)
                            ...
                            P=P_pre2-Sv_current;
                            [~,flag] = chol(reformat_tri(P,y)); %The triangular matrix needs to be reformatted so that chol() can work with it.
                            if flag==0
                                delete_pos(kappa)=1;
                            end
                        end
                    ...
                    N_unchecked=numel(I_eigen_unchecked); %Now we only need to check those matrices we haven't excluded in the step before.
                    N_save=N;
                    cpu_Steps=[1:numMat4Pageeig:N_unchecked N_unchecked]; %Steps for Transmission to GPU
                    isPSD_complete=false(N_unchecked,1); %Allocate memory
                    
                    for a=1:numel(cpu_Steps)-1
                        eigenv=pageeig(reformat_tri(P_pre2-matrices(:,I_eigen_unchecked(cpu_Steps(a):cpu_Steps(a+1))),y));
        ...
        elseif inner_runs>2
            ...
            disp('Start using CO critrion on all remainging matrices...')
    ...
    if not(too_many_vops)
        ...
        save(['VOP_SOR_' name_VOP '_' num2str(eps_G) '.mat'],'VOP','Sglobal','elapsed_time','eps_G','ran_out_counter','overestimation_max','VOPID','timings') %save VOPs in file with a name that tells what was compressed and how.
        Sglobal=format_sq2tri(Sglobal);
    else
    ...
    if iteration_step>1 && estimated_number_of_VOPs > max_num_VOPs %The first iteration does not give a valid result.
        ...
        new_eps_g=10^(log10(max_num_VOPs*0.95)*slope+offset); %Calculate the new overestimation which will most likely result in the desired number of VOPs.
